Shader "Faxime/Introduction/10. Diffuse Ambient Specular Pixel Frac-Texture (Blinn-Phong)"
{
	Properties
	{
		...
		_Fractals("Fractals", Range(1.0, 256.0)) = 5.0
	}
	...
	SubShader
	{
		Pass
		{
			...
			float4 fragment(vertexOutput input) : COLOR
			{
				...
				// Applies a fractal to the current UV coordinate.
				input.UV *= _Fractals;

				//if (input.UV.x < 0.5)
				if (frac(input.UV.x) < 0.2 || frac(input.UV.y) < 0.2)
				{
